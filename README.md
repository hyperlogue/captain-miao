...
| **[opencode](https://github.com/anomalyco/opencode)**               | Has no hooks at all, so a session runs under a synthetic `OPENCODE_CONFIG_DIR` carrying a generated plugin. No resume-picker entries and no worktrees ([known limits](#opencode-support)).                                                                       |
...
- **No resume-picker entries.** `r` on an opencode row lists nothing yet, though resuming a session captain-miao already knows about works. `opencode session list` shows them in the meantime.
