...
/// The header's right cluster is right-aligned, so its ☁️ and host icon slide
/// sideways whenever the tally or a label changes width. ratatui answers a
/// moved VS16 emoji by clearing the column it used to cover, and its crossterm
/// backend prints that cell with no `MoveTo` — one column late, because the
/// emoji already advanced the cursor two. That eats the character next to the
/// glyph and shifts the rest of the run; `pin_wide_cell_widths` is what stops
/// it, so the invariant to hold is that no write ever lands *inside* a glyph.
#[test]
fn a_wide_glyph_never_takes_a_write_in_a_column_it_covers() {
    use ratatui::layout::{Alignment, Rect};
    use ratatui::text::Line;
    use ratatui::widgets::{Paragraph, Widget};
    use unicode_width::UnicodeWidthStr;

    // A header-shaped right-aligned bar: cloud tally, then an icon'd host.
    let bar = |text: &str| {
        ...
        Paragraph::new(Line::from(text.to_string()))
            ...
            .render(area, &mut buf);
        super::draw::pin_wide_cell_widths(&mut buf);
        buf
    };
    // Every way the cluster's width moves: the tally gaining a bucket, a host
    // name growing, the keep-awake ☕ arriving. Each slides the glyphs to its
    // left by a cell or two, onto columns that held a digit last frame.
    let shifts = [
    ...
    for a in shifts {
        for b in shifts {
            let (prev, next) = (bar(a), bar(b));
            // Walk the update stream the way the backend does: it only skips a
            // `MoveTo` when the next cell is the one right after the last, so a
            // cell inside the previous glyph is drawn at the wrong column.
            let mut last: Option<(u16, u16, u16)> = None;
            for (x, y, cell) in prev.diff(&next) {
                if let Some((lx, ly, lw)) = last {
                    assert!(
                        y != ly || x >= lx + lw,
                        "write at {x} lands inside the {lw}-cell glyph at {lx}\n  {a:?}\n  {b:?}",
                    );
                }
                last = Some((x, y, cell.symbol().width().max(1) as u16));
            }
        }
    }
}

/// And the frame really is pinned — the helper above is only worth anything if
/// `App::draw` runs it over what it just drew.
#[test]
fn a_drawn_frame_pins_every_wide_glyph_it_holds() {
    ...
    // The paw alone guarantees one; a frame with none would pass vacuously.
    assert!(wide > 0, "no wide glyph on the frame to check");
}
