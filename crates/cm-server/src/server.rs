...
async fn serve() -> Result<()> {
    // Control socket. Create the parent 0700 and the socket 0600 so another user
    // on the host can't connect (mirrors the launcher socket).
        ...
        tokio::select! {
            ...
            accepted = listener.accept() => {
                ...
                tokio::spawn(async move {
                    let _guard = guard;
                    if let Err(e) = handle_conn(stream, backend, changes, host).await {
...
async fn handle_conn(
    ...
    // Expect Hello first; reply Welcome. A protocol mismatch still gets a
    ...
    let mut last_sent: HashMap<u32, LauncherState> = HashMap::new();
            ...
            frame = read_frame::<_, ClientFrame>(&mut rd) => {
                ...
                match frame {
                    ...
                    ClientFrame::Subscribe => {
                        ...
                        last_sent = sessions.iter().map(|s| (s.launcher_pid, s.clone())).collect();
                    ...
                    ClientFrame::KillSession { req_id, child_pid } => {
                        let ok = backend.kill_session(child_pid);
                    ...
                    ClientFrame::ListRecentDirs { req_id } => {
                        let (cwds, home) = backend.recent_dirs();
                        write_frame(&mut wr, &ServerFrame::RecentDirs { req_id, cwds, home }).await?;
                    }
...
async fn push_changes(
    ...
    last_sent: &mut HashMap<u32, LauncherState>,
) -> std::io::Result<()> {
    let cur: HashMap<u32, LauncherState> = backend
        .list_sessions()
        .into_iter()
        .map(|s| (s.launcher_pid, s))
        .collect();
    for &pid in last_sent.keys() {
        if !cur.contains_key(&pid) {
            write_frame(wr, &ServerFrame::Removed { launcher_pid: pid }).await?;
        }
    }
