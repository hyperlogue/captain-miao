...
A new session can be launched into an isolated git **worktree** — `Ctrl-g` in
the `O` cwd picker, per-launch, armed state shown on the popup's own bottom line
(`Worktree new`).

**captain-miao does not create, name, or clean up worktrees, and must not start.**
The whole feature is one flag on the launch argv: `AgentControl::worktree_args`
contributes `--worktree [name]`, `OpenSpec.worktree` carries the request over the
seam (and the wire — `#[serde(default)]`, additive), and `LocalBackend::open_session`
appends it. Everything else is the agent's: Claude Code creates the worktree
under `.claude/worktrees/<name>/` on a new branch, honours `worktree.baseRef`
(`fresh` = the remote default branch, `head` = local HEAD) and `.worktreeinclude`
(copying gitignored `.env`-style files in), **blocks** edits/commands/git
redirects that would reach the main checkout, prompts to keep-or-remove on exit,
and re-enters the worktree on resume. Reimplementing any of that here would be a
second, disagreeing copy of a thing the agent already does better — and the
isolation enforcement in particular is not something a launcher can bolt on.

Consequences of the agent owning it, each of which is why a piece of code looks
the way it does:

- **Resume and restart never pass the flag.** The agent returns a resumed
  session to its own worktree from its session binding, so asking again would
...
- **Codex has no worktree concept** (0.147: no such flag), so `worktree_args`
  ...
  matched separately, so the UI gate and the argv can't disagree. The picker
  hides the key for Codex, `Ctrl-t` onto Codex **disarms** a pending request
  rather than holding it invisibly, and `open_session` drops an impossible
  request rather than failing the launch (losing isolation is recoverable;
  refusing to open a session the user asked for is not).
- **The key is `Ctrl-g` ("git"), not the obvious `Ctrl-w`.** The picker's path
  input binds `Ctrl-w` to readline delete-previous-word (pinned by
  `picker_readline_ctrl_w_deletes_prev_word`), along with `a`/`e`/`b`/`f`/`d`/
  `u`/`k` — which is also why the agent and host switches sit on `Ctrl-t`/`Ctrl-h`.
- **`o` never opens a worktree.** It is the no-questions path (same cwd, straight
  to a window); isolation is a decision, so it lives on `O`'s picker where it is
  visible before Enter. Nothing about it is persisted either — `Space a`/`Space H`
  answer "what do I usually use", while isolation answers "is *this* task one
...
`tmux:<socket>,<server-pid>` matches and the binding would **not** be classified
foreign; it would be treated as ours and then never resolve, since `snapshot()`
is `list-panes -s -t $session` scoped to the dashboard's own session. That is
strictly worse than the dimmed-foreign path, which at least degrades honestly.
...
- **Config.** Hosts are mutable TUI state (`hosts.json`: label, ssh target /
  socket, emoji icon, suspended flag), managed via the `Space h` **hosts panel** —
  a list view with live conn state, running/attached counts, daemon version and
  latency. There is no Save step: adding persists + connects immediately, edits
  apply on row commit, removal is behind a `y/N`, and `c` suspends/resumes a host
  in place (see Key bindings). A row commit **only reconnects when the
  connection actually changed** (`conn_identities` — label + target + the
  suspended flag, the three things a backend is built from): an icon-only edit
  used to drop and re-dial every host, which is a multi-second storm for a
  cosmetic change and reset the auto-reattach epochs besides. Suspension is on
  that list rather than beside it because it is the same question — a suspended
  host builds *no* backend, so `build_backends_from_config` skips it after
  registering its icon, and toggling it has to reach `rebuild_remote_backends`.
  Its attach windows are deliberately left alone: the far sessions keep running,
  the bindings prune on live windows rather than on live hosts, and resuming the
  host finds them still bound. There is no per-host **colour** — the emoji says
